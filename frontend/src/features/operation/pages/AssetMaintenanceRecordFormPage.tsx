...
export function AssetMaintenanceRecordFormPage() {
  ...
  return <div className="space-y-4"><div className="flex items-center justify-between"><h1 className="text-xl font-semibold">{t('create', { ns: 'operations' })} {t('maintenanceRecord', { ns: 'operations' })}</h1><Link to="/operations/asset-maintenance-records" className="text-sm text-violet-600">{t('back', { ns: 'operations' })}</Link></div><AssetMaintenanceRecordForm isSubmitting={mutation.isPending} submitLabel={t('create', { ns: 'operations' })} onSubmit={(values)=>mutation.mutate(values,{onSuccess:(created)=>{toast.success(t('createSuccess', { ns: 'operations' }));navigate(`/operations/asset-maintenance-records/${created.id}`)},onError:(err)=>toast.error(getErrorMessage(err))})} /></div>
}
