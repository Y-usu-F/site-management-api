...
export function AssetMaintenanceRecordFormPage() {
  ...
  return <div className="space-y-4"><div className="flex items-center justify-between"><h1 className="text-xl font-semibold">{t('operations.common.create')} {t('operations.common.maintenanceRecord')}</h1><Link to="/operations/asset-maintenance-records" className="text-sm text-violet-600">{t('operations.common.back')}</Link></div><AssetMaintenanceRecordForm isSubmitting={mutation.isPending} submitLabel={t('operations.common.create')} onSubmit={(values)=>mutation.mutate(values,{onSuccess:(created)=>{toast.success(t('operations.common.createSuccess'));navigate(`/operations/asset-maintenance-records/${created.id}`)},onError:(err)=>toast.error(getErrorMessage(err))})} /></div>
}
