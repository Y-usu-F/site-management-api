...
export function ServiceRequestFormPage({ mode }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between"><h1 className="text-xl font-semibold">{mode === 'create' ? 'New service request' : 'Edit service request'}</h1><Link to="/operations/service-requests" className="text-sm text-violet-600">Back</Link></div>
      <ServiceRequestForm
        ...
        onSubmit={(values) => {
          if (mode === 'create') {
            createMutation.mutate(values, {
              onSuccess: (created) => { toast.success('Service request olusturuldu.'); navigate(`/operations/service-requests/${created.id}`) },
              onError: (err) => toast.error(getErrorMessage(err)),
            })
          } else if (parsedId !== null) {
            updateMutation.mutate({ id: parsedId, body: values }, {
              onSuccess: () => { toast.success('Service request guncellendi.'); navigate(`/operations/service-requests/${parsedId}`) },
              onError: (err) => toast.error(getErrorMessage(err)),
            })
          }
        }}
