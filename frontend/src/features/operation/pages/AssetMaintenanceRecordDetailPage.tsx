...
export function AssetMaintenanceRecordDetailPage() {
  ...
  return <div className="space-y-4"><div className="flex items-center justify-between"><h1 className="text-xl font-semibold">Maintenance record #{row.id}</h1><Link to="/operations/asset-maintenance-records" className="text-sm text-violet-600">Back</Link></div><OperationActionButtons entity="asset_maintenance_record" id={row.id} /><div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800"><div>Asset: {row.asset_id}</div><div>Plan: {row.maintenance_plan_id ?? '-'}</div><div>Performed at: {row.performed_at}</div><div>Cost: {row.cost_amount ?? '-'} {row.currency ?? ''}</div><div>Status: {row.status ?? '-'}</div><div>Description: {row.description ?? '-'}</div></div></div>
}
