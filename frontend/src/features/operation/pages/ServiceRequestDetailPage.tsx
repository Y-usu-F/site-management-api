...
export function ServiceRequestDetailPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between"><h1 className="text-xl font-semibold">{t('serviceRequest', { ns: 'operations' })} #{row.id}</h1><div className="flex gap-3 text-sm"><Link to="/operations/service-requests">{t('back', { ns: 'operations' })}</Link>{canUpdate ? <Link to={`/operations/service-requests/${row.id}/edit`} className="text-violet-600">{t('edit', { ns: 'operations' })}</Link> : null}</div></div>
      <OperationActionButtons entity="service_request" id={row.id} />
      <div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
        <div>{t('title', { ns: 'operations' })}: {row.title}</div><div>{t('description', { ns: 'operations' })}: {row.description}</div><div>{t('site', { ns: 'operations' })}: {siteMap[row.site_id] ?? `#${row.site_id}`}</div><div>{t('unit', { ns: 'operations' })}: {row.unit_id ? (unitMap[row.unit_id] ?? `#${row.unit_id}`) : '-'}</div><div>{t('resident', { ns: 'operations' })}: {row.resident_profile_id ? (residentMap[row.resident_profile_id] ?? `#${row.resident_profile_id}`) : '-'}</div><div>{t('priority', { ns: 'operations' })}: {row.priority ?? '-'}</div><div>{t('status', { ns: 'operations' })}: <OperationStatusBadge status={row.status} /></div>
      </div>
    </div>
  )
}
