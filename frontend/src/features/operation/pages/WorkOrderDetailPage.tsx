...
export function WorkOrderDetailPage() {
  ...
  return <div className="space-y-4"><div className="flex items-center justify-between"><h1 className="text-xl font-semibold">Work order #{row.id}</h1><div className="flex gap-3 text-sm"><Link to="/operations/work-orders">Back</Link>{canUpdate ? <Link to={`/operations/work-orders/${row.id}/edit`} className="text-violet-600">Edit</Link> : null}</div></div><OperationActionButtons entity="work_order" id={row.id} /><div className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800"><div>Service request: #{row.service_request_id}</div><div>Assigned user: {row.assigned_to_user_id ? `#${row.assigned_to_user_id}` : '-'}</div><div>Vendor: {row.vendor_name ?? '-'}</div><div>Start: {row.planned_start_at ?? '-'}</div><div>End: {row.planned_end_at ?? '-'}</div><div>Cost: {row.cost_amount ?? '-'} {row.currency ?? ''}</div><div>Status: <OperationStatusBadge status={row.status} /></div><div>Notes: {row.notes ?? '-'}</div></div></div>
}
